...
class RequestHandler():
    ...
    async def __call__(self, request):
        kw = None
        if self._has_named_kw_args or self._has_var_kw_args or self._required_kw_args:
            if request.method == 'POST':
                ...
                if ct.startswith('application/json'):
                    ...
                    kw = params
                elif ct.start_with('application/x-www-form-urlencoded') or ct.start_with('multipart/form-data'):
                    params = await request.post()
                    kw = dict(**params)
                else:
                    raise web.HTTPBadRequest('Unsupported Content-Type: %s' % request.content_type)
        ...
        if self._required_kw_args:
            ...
            for name in self._required_kw_args:
                logging.info(name)
                if not name in kw:
                    raise web.HTTPBadRequest('Missing argments: %s' % name)
...
def manage_static(app):
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static')
    app.router.add_static('/manage/static/', path, show_index=True)
    logging.info('add manage static %s => %s' % ('/static', path))
